...
T = {
 ...
 "th": {"en": ["Form","Damage","Layer","Class","Reaches the graph","Status","Example"],
        "fr": ["Forme","Dégât","Couche","Classe","Atteint le graphe","Statut","Exemple"]},
 "st": {"en": {"validated": "validated", "proposed": "proposed", "migrated_unreviewed": "unreviewed", "refuted": "refuted"},
        "fr": {"validated": "validée", "proposed": "proposée", "migrated_unreviewed": "non relue", "refuted": "réfutée"}},
...
# The flag of the language the switch takes you to. circle-flags (HatScripts), MIT — a real icon
# set, so the Union flag has its diagonals offset, which a hand-drawn one does not.
FLAG = """<svg width="18" height="18" aria-hidden="true" class="fg" viewBox="0 0 512 512"><mask id="flg-fr"><circle cx="256" cy="256" r="256" fill="#fff"/></mask><g mask="url(#flg-fr)"><path fill="#eee" d="M167 0h178l25.9 252.3L345 512H167l-29.8-253.4z"/><path fill="#0052b4" d="M0 0h167v512H0z"/><path fill="#d80027" d="M345 0h167v512H345z"/></g></svg>""" if LANG == "en" else """<svg width="18" height="18" aria-hidden="true" class="fg" viewBox="0 0 512 512"><mask id="flg-gb"><circle cx="256" cy="256" r="256" fill="#fff"/></mask><g mask="url(#flg-gb)"><path fill="#eee" d="m0 0 8 22-8 23v23l32 54-32 54v32l32 48-32 48v32l32 54-32 54v68l22-8 23 8h23l54-32 54 32h32l48-32 48 32h32l54-32 54 32h68l-8-22 8-23v-23l-32-54 32-54v-32l-32-48 32-48v-32l-32-54 32-54V0l-22 8-23-8h-23l-54 32-54-32h-32l-48 32-48-32h-32l-54 32L68 0H0z"/><path fill="#0052b4" d="M336 0v108L444 0Zm176 68L404 176h108zM0 176h108L0 68ZM68 0l108 108V0Zm108 512V404L68 512ZM0 444l108-108H0Zm512-108H404l108 108Zm-68 176L336 404v108z"/><path fill="#d80027" d="M0 0v45l131 131h45L0 0zm208 0v208H0v96h208v208h96V304h208v-96H304V0h-96zm259 0L336 131v45L512 0h-45zM176 336 0 512h45l131-131v-45zm160 0 176 176v-45L381 336h-45z"/></g></svg>"""
...
<nav><a href="{p}index.html#forms">{E(t("nav_forms"))}</a><a href="{p}index.html#about">{E(t("nav_about"))}</a><a href="{DATA}">{E(t("nav_data"))}</a><a href="{REPO}">GitHub</a><a href="https://doi.org/{DOI}">DOI</a><a class="lang" href="{SWITCH[depth]}" title="{E(t("switch_title"))}" aria-label="{E(t("switch_title"))}">{FLAG}</a></nav></header>
<main>{body}</main>
...
<td>{E(f['layer'])}</td><td>{E(CLASS_SHORT[f['class']])}</td><td class="reach r-{'no' if upstream(f) else 'yes'}">{E(t('no') if upstream(f) else t('yes'))}</td><td class="st st-{f['status']}">{E(t('st').get(f['status'], f['status']))}</td><td class="proof p-{proof_of(f)[0]}">{E(proof_of(f)[1])}</td></tr>""" for f in forms)
...
TH = "".join(('<th data-sort="text" class="sorted-asc">' if i==0 else '<th data-sort="text">')+html.escape(h)+"</th>" for i,h in enumerate(t("th")))
...
ths.forEach((t,i)=>t.addEventListener('click',()=>sortBy(i,i===sortCol?-sortDir:1)));
sortBy(0,1);
...
td.proof{font-size:13px}td.p-docs{color:#166534;font-weight:600}td.p-none{color:var(--muted)}td.reach,td.st{font-size:13px}td.r-yes{font-weight:600}td.r-no{color:var(--muted)}td.st-validated{color:#166534;font-weight:600}td.st-migrated_unreviewed,td.st-proposed{color:var(--muted)}td.st-refuted{color:#991b1b}label.chk{display:inline-flex;align-items:center;gap:6px;font-size:14px;color:var(--ink)}label.chk input{width:auto;min-width:0;flex:none}table.kv th{width:190px;text-transform:none;letter-spacing:0;font-size:14px;color:var(--ink);background:transparent;border-top:1px solid var(--line)}table.kv td code{font-size:12.5px;white-space:normal;word-break:break-all}td.mono{font-family:var(--mono);font-size:12px}
...
.lang{display:inline-flex;align-items:center;padding:2px}.lang svg.fg{display:block;opacity:.85;transition:opacity .15s,transform .15s}.lang:hover svg.fg,.lang:focus-visible svg.fg{opacity:1;transform:scale(1.08)}

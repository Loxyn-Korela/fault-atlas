...
# What the fault costs at the end of the chain, in the terms of someone searching. From the
# severity column of the internal catalogue of 2026-08-09, recovered 2026-09-10.
SEVERITY = {
 "G1": ("Contaminates", "#9B2C2C",
        "The error lands on an entity, and every fact hanging from that entity inherits it. One wrong identity, and a whole neighbourhood of the graph answers wrongly."),
 "G2": ("Answers wrongly, in silence", "#8A4B12",
        "A single false fact, local, and nothing signals it. You get an answer, it looks like every other answer, and it is wrong. This is the one a low prevalence must not excuse."),
 "G3": ("Hides", "#1F4E79",
        "A hole. The graph is incomplete, not lying. What you are looking for is simply not there, and nothing tells you it should have been."),
 "G4": ("Stops before the graph", "#5B6670",
        "The fault never reaches the graph. It costs work upstream, not answers downstream."),
}
CLASS_SHORT = {"DEFEATED": "Cancelled by code", "DEFEATED_IF_XML": "Cancelled if XML", "REFUSABLE": "Can abstain",
               "SILENT_FALSE": "Silent false", "IRREDUCIBLE": "Irreducible", "UNCLASSIFIED": "Unclassified"}
...
<select id="fs"><option value="">Any consequence</option><option value="G1">Contaminates</option><option value="G2">Answers wrongly, in silence</option><option value="G3">Hides</option><option value="G4">Stops before the graph</option><option value="__none__">Consequence not judged</option></select>
...
<div class="tablewrap"><table id="t"><thead><tr><th data-sort="text">Form</th><th data-sort="text">Damage</th><th data-sort="text" class="sorted-desc">Date</th><th data-sort="text">Class</th><th data-sort="text">Layer</th><th data-sort="text">Corpus</th><th data-sort="text">Example</th></tr></thead><tbody>{rows}</tbody></table></div></section>

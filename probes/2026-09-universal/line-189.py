...
@sonde(189, "a script that carries no case, so every capital-based rule goes blind")
def d189(x):
    t = _plain(x)
    n = sum(1 for c in t if "ऀ" <= c <= "ॿ" or "؀" <= c <= "ۿ"
            or "가" <= c <= "힯" or "฀" <= c <= "๿")
    return [f"{n} characters in a caseless script"] if n > 20 else []

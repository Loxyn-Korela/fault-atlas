...
## 0.23.0 — 2026-09-13
- **The first eight forms this atlas did not find itself**, and the first that credit somebody else. 171 forms carried our own organisation on every one; that was an omission, not a claim, and a prior-art sweep corrected it. **form-200** run-on, a space the writing had and the reading lost (Nguyen et al., JCDL 2019 — and only 19.25 % of named entities touched by a segmentation error survive recognition). **form-201** real-word error, a valid word and the wrong one, invisible to every lexical check — **59.21 % of OCR errors**, the majority and not the margin, where our form-023 shows only non-words. **form-202** the first character, where the same substitution that is benign mid-token leaves **30.41 %** of entities recognised against 79.5 % elsewhere (Hamdi et al., NLE 2022). **form-203** a merged header cell destroyed in the markup, which changes nothing on screen because the borders are invisible, and **form-204** one logical column split in two so the figures align, so the value keeps its digits and loses its unit (Smock et al., CVPR 2022 and ICDAR 2023). **form-205** a document that determines no reading order, where the right answer is an abstention and a reader produces an order anyway (Clausner et al., ICDAR 2013). **form-206** nested entities, where every boundary is defensible and they exclude each other, one of the rare cases where the ground truth itself is under-determined, and **form-207** annotating the first mention only, which loses no fact and divides every mention-weighted edge (van Erp et al., LREC 2016).
- Each carries its finders in `origin.organisation` and their own definition, quoted verbatim, as its excerpt. The damage and the layer are **our** reading of their fault and were not stated by them; the history field says so on each.
- **And four antecedents of the framework itself, named in the README.** Merge and split have a formal semantics and a configurable cost model since Menestrina et al., PVLDB 3(1), 2010. Weighting a fault by its downstream consequence was published as an *allowable / non-allowable* flag by Clausner et al. in **2011**, fourteen years before LED and closer to this atlas than LED. Our `REFUSABLE` class is Fellegi-Sunter's no-decision region, via Winkler 2006, which unlike ours fixes its thresholds from a-priori error bounds. And the MUC categories carry two outcomes our seven damages cannot express, *partial* and *noncommittal*.
- **What the sweep did not find, and it is why this atlas exists:** no published catalogue classifies faults of documents by the effect they have on a graph. Six bodies of work hold half that ground each; none holds both. The README says which, and says it before anyone asks.

## 0.22.0 — 2026-09-13
- **A single "not reachable by deletion" was hiding two obstacles, and they call for opposite work.** 154 forms now carry `repair.obstacle`, and it says which: **the information** means deletion is the right gesture and what is missing is what tells the true candidate from the false one — feed the repairer and it works, measured at 130 of 130 against a coin's 65. **The operation** means deletion cannot express the repair at all: mending a gap means adding, a wrong value means setting, a merge means splitting, and a deletion-only repair's solution is a set of elements to delete with no insertion anywhere in it. There, no weight and no information change the answer.
- 120 forms are blocked by the operation, 20 by the information, 14 are reached. The split matters because it says what a repair researcher would have to *build* rather than what merely fails: a reader who sees "not reachable" on 112 forms learns nothing about which of the two to attempt.
- Derived per damage class, from what a deletion-only repair can express — read in the repairer's own code, whose solution class is named for deletion and carries no insertion — together with the measurement of 2026-09-10. The source says so on every form.
- It also corrects something we had been saying: that the twelve thousand unreached damages were out of reach "because the information the graph does not contain". That is true of one class of six. Both memos are corrected.

## 0.21.0 — 2026-09-13
- **form-199, found by a damage generator for documents on its first run, and found in our own reader.** The generator damaged both `<pub-date>` elements of an article; the reader returned the true year anyway, caught at character 8,952 of 106,654 inside `<history><date date-type="received">`. It was reporting the date a manuscript was RECEIVED as the date it was PUBLISHED, and it went unnoticed because the two usually share a year. 118 documents of 200.
- **The reader's fault is not an atlas form** — the atlas records what a corpus carries, not what an instrument commits, and that rule already sent one candidate to `docs/not-entered-journal-2026-09.json` yesterday. What enters is the property of the corpus that makes the mistake possible and silent: JATS gives publication, reception, acceptance and revision the same element name, `<year>`, and distinguishes them only by what contains them.
- Consequence proposed as G2, and the reason is the shape of the error rather than its rate: the substituted value is a well-formed date of the right magnitude and usually the right one, and it is wrong exactly for the documents where reception and publication fall in different years — a systematic subset, not a random one. A study of when something was first published is a study of boundaries, and this is wrong on the boundaries.

## 0.20.0 — 2026-09-13
- **Four more forms from the cleaning journal, and a rule of entry corrected.** They were held back on the ground that they had a figure but no named object. That was wrong, and it was a confusion between *a named object* and *an openable document*: for these four the object is a file or a declared convention of the **producer** — a dictionary, a README, two delivery files, a way of writing a year — and not a record of the corpus. That is exactly what makes them faults of a registry, with an extent rather than a prevalence, and the atlas has had the field for them since 0.14.
- **form-195** a field missing on 57.4 % of passages because the base has two encodings, not because it is missing. The signature is a pair of rigorously equal counts, 214,982 `title` against 214,982 `abstract`, with zero mixed document in 323,671 read. This is the most dangerous shape in corpus work and the only one that leaves no trace: discarding the records whose field is empty is the obvious, defensible move, and here it keeps 1.1 % of the corpus — open access, recent, thematically biased — with nothing going red.
- **form-196** one producer, two vocabularies for the same field across two files of one delivery: `mutation2pubtator3.gz` writes one label, `relation2pubtator3.gz` writes four. 363,749 distinct entities looked absent from a set they were in.
- **form-197** the producer's own documentation under-declares its vocabulary: 18 RefTypes declared against 20 present, 8 relation types against 12, 20 sections against 41, 12 annotation types against 17. Four of the five counts rose again between five archives and ten.
- **form-198** one date field, four ways of writing a year. A reader built for one shape found 12.5 % of them, and was caught only because 12.5 % is absurd on its face; a shape covering 90 % would have passed in silence.
- **And the four that do not enter, with their reasons, in `docs/not-entered-journal-2026-09.json`** — an HTTP error page written as data (no named object, and a fault of a transfer rather than of a corpus), a type leak measured at 3 passages in a billion (counts only), three declared-but-unused labels (named and exact, and still not a fault: the journal's own decision was to keep them), and a query missing the official MeSH name (a fault of our instrument, the same class as the 26 catalogue lines ruled out of scope in August). Two of them say what would make them enter.

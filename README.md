...
167 forms from six observation campaigns. The counts below are the `origin.campaign` field of the records themselves, not a tally kept by hand:
...
- **September 2026, third wave** — 1 form, `form-208`, a claim attributed to another source and written as a fact: 60 of the 272 articles carry one, 7 deny it in the next sentence. It closes a gap: denial and reported speech were fabricated in our test corpora with no observed form behind them. The same morning, `form-042` (names damaged by scanning), a declared hypothesis since the migration, received its first specimen — five author names in three of the 45 migraine PDFs, found by probe.
